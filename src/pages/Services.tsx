...
export default function Services() {
  ...
  useEffect(() => {
    ...
    // New/Old request modal - skip if user came from Home resume
    useEffect(() => {
      if (!isDataLoaded) return;
      const hasChosen = sessionStorage.getItem('jadwali_choice_made');
      const resumeNavigated = sessionStorage.getItem('jadwali_resume_navigated');
      if (resumeNavigated === 'true') {
        // User came from Home "resume previous", skip this modal
        sessionStorage.removeItem('jadwali_resume_navigated');
        setChoiceMade(true);
        return;
      }
  ...
  // ✅ دالة مساعدة لبناء بيانات الإرسال الكاملة
const buildSubmitData = (status: string = 'جديد', whatsapp: string = 'لا', submit: string = 'لا'): Record<string, unknown> => {
  const allSubjectsList = [...subjectsData, ...customSubjects];
  ...
  return {
    ...
    institutionName: getFormString('institutionName'),
    phone: getFormString('phone'),
    region: selectedRegion === 'other' ? getFormString('otherRegion') : selectedRegion,
    otherRegion: selectedRegion === 'other' ? getFormString('otherRegion') : '',
    province: selectedProvince === 'other' ? getFormString('otherProvince') : selectedProvince,
    otherProvince: selectedProvince === 'other' ? getFormString('otherProvince') : '',
    commune: selectedCommune === 'other' ? otherCommuneText : selectedCommune,
    ...
    // التعديلات والحالات الخاصة
    modifications,
    modifyHours,
    specialCaseNotes: specialCaseNotes || '',
    editFileLink: editFileLink || '',
    
    // التوقيتات
    morningStart: getFormString('morningStart', '08:00'),
    morningEnd: getFormString('morningEnd', '12:00'),
    eveningStart: getFormString('eveningStart', '14:00'),
    eveningEnd: getFormString('eveningEnd', '18:00'),
    continuousTime,
    nonWorkDays: getFormString('nonWorkDays', ''),
    
    // الملاحظات والوثائق
    teacherConditions: getFormString('teacherConditions', ''),
    orgWishes: getFormString('orgWishes', ''),
    additionalNotes: getFormString('additionalNotes', ''),
    supportFilesLinks: supportFilesLinks || '',
...
// ✅ دالة مساعدة لإرسال البيانات إلى Google Sheet (بدون تحقق)
...
// ✅ دالة نسخ المراجعة وإرسالها للواتساب
const handleCopyReview = async () => {
  ...
  // 2. إرسال البيانات إلى Google Sheet مع حالة "واتساب" = نعم، "إرسال" = لا
  const submitData = buildSubmitData('واتساب', 'نعم', 'لا');
  sendToGoogleSheet(submitData);

  // 3. إشعار المستخدم فوراً
  alert('✅ تم نسخ جميع المعلومات بنجاح!.\n\nيمكنك الآن لصقها في واتساب.');
};

// ✅ دالة إرسال الطلب الرئيسي
const handleSubmit = async (e: React.FormEvent) => {
  e.preventDefault();
  
  // بناء البيانات مع "واتساب" = لا، "إرسال" = نعم
  const submitData = buildSubmitData('جديد', 'لا', 'نعم');
  
  // إرسال البيانات (بدون انتظار)
  sendToGoogleSheet(submitData);

  // مسح البيانات وإظهار نافذة النجاح فوراً
  localStorage.removeItem(STORAGE_KEY);
  setShowSuccess(true);
};
            ...
            <form ref={formRef} onSubmit={handleSubmit}>
              ...
              {currentStep === 5 && (
                <div className="form-step active">
                  ...
                  <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)' }}>
                    ...
                    {generalRoomsCount > 0 && <div className="form-group"><label>📚 أسماء وأرقام القاعات العامة ({generalRoomsCount} قاعة)</label><textarea rows={3} placeholder="مثال: من قاعة 1 الى 12 ثم من 16 الى 19..." value={generalRoomNamesText} onChange={(e) => setGeneralRoomNamesText(e.target.value)} /></div>}
                    {scienceRoomsCount > 0 && <div className="form-group"><label>🔬 أسماء وأرقام المختبرات العلمية ({scienceRoomsCount} مختبر)</label><textarea rows={3} placeholder="مثال: مختبر pc2 مختبر svt1..." value={scienceRoomNamesText} onChange={(e) => setScienceRoomNamesText(e.target.value)} /></div>}
                ...
                <div className="form-step active">
                  <div className="step-header">
                    ...
                    <div>
                      ...
                      <p className="step-desc">حدد إن كنت تريد تفويج الأقسام للمواد العلمية </p>
                    </div>
                  </div>
              ...
              {currentStep === 8 && (
                <div className="form-step active">
                  ...
                  {serviceType === 'edit' && <div style={{ marginTop: '1.5rem' }}><div className="form-group"><label>رابط الملف (Google Drive أو واتساب)</label><input type="text" name="editFileLink" placeholder="أدخل رابط الملف أو أرسله عبر واتساب 0651011102" value={editFileLink} onChange={(e) => setEditFileLink(e.target.value)} /></div></div>}
                  {serviceType === 'special' && <div style={{ marginTop: '1.5rem' }}><div className="form-group"><label>اشرح الحالة الخاصة بالتفصيل</label><textarea name="specialCaseText" rows={5} placeholder="صف الوضعية الخاصة أو الإشكالية التي تواجهها او تواصل معنا عبر الواتساب 0651011102..." value={specialCaseNotes} onChange={(e) => setSpecialCaseNotes(e.target.value)}></textarea></div></div>}
                </div>
              )}
              ...
              {currentStep === 9 && (
                <div className="form-step active">
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-group full-width"><label>أيام أو فترات عدم العمل</label><textarea name="nonWorkDays" rows={2} placeholder="مثال: السبت مساءا، الجمعة بعد الزوال حسب الامكانية..." value={getFormString('nonWorkDays')} onChange={handleInputChange}></textarea></div>
                  </div>
                </div>
              )}
